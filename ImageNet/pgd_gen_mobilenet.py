...
def get_n_images(n, loss_func, ds):
    ...
    for i, features in enumerate(ds):
        for j in range(BATCH_SIZE):
            if os.path.exists("/local/rcs/wei/attack_baseline/mobilenet/baseline_gen_" + loss_func + "/"+str(num)+"_filter.npy"):
                num += 1
                continue
            ...
            suc_cnt += success
            np.save("/local/rcs/wei/attack_baseline/mobilenet/baseline_gen_" + loss_func + "/"+str(num)+"_filter.npy", fil)
            if indicator == 0:
